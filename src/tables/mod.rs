...
pub fn deco_table() -> serde_json::Result<types::TableAirDeco> {
  let mut file = File::open("src/tables/JSON/usnavy-air-deco-rev7.json")
...
pub fn rgl_table() -> serde_json::Result<types::TableRgl> {
  let mut file = File::open("src/tables/JSON/usnavy-air-repetgroup-rev7.json")
...
pub fn rnt_table() -> serde_json::Result<types::TableRnt> {
  let mut file = File::open("src/tables/JSON/usnavy-air-rnt-rev7.json")
